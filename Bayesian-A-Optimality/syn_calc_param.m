...
ratio_param_names={'\gamma', '\gamma^G'}; % \gamma is not calculated
...
ratio_calced = 0; %  not calc again by default
...
if 0 == cur_calced
    for exp_id = 1:nm_exps
        ...
        cur_pm(exp_id) = calc_cur_pm(f, pm(exp_id), xs_greedy, fs_fr, x_tra_fr,   max_k);
        %  slow, so only run once.    cur:  total cur
    end
